...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(vals) != len(formals):
            raise SchemeError("Number of formal parameters does not equal the number of scheme values!")
        while formals:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target.first, LambdaProcedure(formals, expressions.second, env))
        return target.first
    else:
...
def do_if_form(expressions, env):
    ...
    elif not expressions.second.second:
        return None
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions.second:
        item = scheme_eval(expressions.first, env)
        if scheme_false(item):
            return False
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        vals, formals = (Pair(scheme_eval(binding.second.first, env), vals)), (Pair(binding.first, formals))
...
def check_formals(formals):
    ...
    while formals:
        ...
        if formals.first in params:
            raise SchemeError("A symbol is repeated in the parameters!")
        else:
            params += [formals.first]
        formals = formals.second
    return
